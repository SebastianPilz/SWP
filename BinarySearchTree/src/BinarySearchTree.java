...
public class BinarySearchTree {
	...
	public Node add(Node n, Node pos) {
		if (head == null) {
			// Head wird gesetzt
			head = n;
			...
			// Erste Version
			/*
			 * // Head ist schon gesetzt Node tmp = head;
			 * 
			 * while (tmp.getBigger() != null && n.getValue() > tmp.getValue() ||
			 * tmp.getSmaller() != null && n.getValue() <= tmp.getValue()) { // Finden der
			 * letzten Zahl, bzw wo der richtige Platz ist
			 * 
			 * if (n.getValue() <= tmp.getValue()) { tmp.setSmall(); tmp = tmp.getSmaller();
			 * } else { tmp.setBig(); tmp = tmp.getBigger(); } }
			 * 
			 * if (n.getValue() <= tmp.getValue()) { tmp.setSmaller(n); tmp.setSmall();
			 * System.out.printf("Eingesetzt: %d \n", n.getValue()); } else {
	...
	public Node show(Node n) {
		if (n.getBig() == 0 && n.getSmall() == 0) {
			System.out.println(n.getValue());
			return n;
		}

		System.out.println(n.getValue());
		return show(n.getSmaller());
	}
	...
	public Node remove(Node n, Node pos, Node parent) {
		...
		if (n.getValue() == pos.getValue()) {
			// Zu löschende Node gefunden
			System.out.println("Da habn man!");
			if (pos.getBig() == 1) {
				// Es gibt nur einen groesseren Wert unter der Node
				...
				// Es gibt keinen groesseren Wert unter der Node
				System.out.println("Der hat koanen");
				if (pos.getSmaller() == null) {
					// Es gibt keinen kleineren Wert unter der Node
					// Loeschen der Node
					if (pos.getValue() < parent.getValue()) {
					...
					// Es gibt mehrere kleinere Werte unter der zu loeschenden Node
				...
				// Es gibt mehrere groessere Werte unter der zu loeschenden Node
